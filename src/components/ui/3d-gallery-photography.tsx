...
function GalleryScene({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      ...
      // If not looping and we've reached the end/beginning, allow default scroll
      if (!loop) {
        // Use a small threshold to make it feel less "sticky" at the exact boundary
        if (isScrollingDown && accumulatedScroll >= depthRange - 0.01) return;
        if (!isScrollingDown && accumulatedScroll <= 0.01) return;
      }

      event.preventDefault();
      const velocity = event.deltaY * 0.01 * speed;
      setScrollVelocity((prev) => prev + velocity);
      setAutoPlay(false);
      lastInteraction.current = Date.now();
    },
    [speed, loop, accumulatedScroll, depthRange],
  );
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (!loop) {
        if (isMovingForward && accumulatedScroll >= depthRange - 0.01) return;
        if (isMovingBackward && accumulatedScroll <= 0.01) return;
      }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Date.now() - lastInteraction.current > 3000) {
        // Only auto-play if looping or not yet at the end
        if (loop || accumulatedScroll < depthRange - 1) {
          setAutoPlay(true);
        }
      }
    }, 1000);
    return () => clearInterval(interval);
  }, [loop, accumulatedScroll, depthRange]);
  ...
  useFrame((state, delta) => {
    ...
    const frameVelocity = scrollVelocity * delta * 10;

    // Track accumulated scroll for non-looping boundary logic
    if (!loop) {
      setAccumulatedScroll((prev) => {
        const next = prev + frameVelocity;

        // If we hit the boundary, kill the velocity to stop animation immediately
        // and prevent "overshoot" feeling
        if (next >= totalRange) {
          setScrollVelocity(0);
          setAutoPlay(false);
          return totalRange;
    ...
    planesData.current.forEach((plane, i) => {
      if (loop) {
        let newZ = plane.z + frameVelocity;
        let wrapsForward = 0;
        let wrapsBackward = 0;
        ...
        // In non-looping mode, planes maintain their relative spacing
        // Based on their initial index within the depth range
        const initialZ = visibleCount > 0 ? (totalRange / visibleCount) * i : 0;
        let newZ = initialZ + accumulatedScroll;

        // Wrap for vertical distribution but NOT for depth progress
        // Actually, we want them to just move together.
        // The image index should stay fixed for each plane in non-loop mode.
        plane.z = newZ;
      }
